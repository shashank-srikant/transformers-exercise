...
def add_lora_adapter(input_linear_layer, alpha, rank):
    ...
    """
    input_linear_layer.A = nn.Parameter(
        torch.randn((input_linear_layer.in_features, rank))
    )
    input_linear_layer.B = nn.Parameter(
        torch.zeros((rank, input_linear_layer.out_features))
    )
...
]  # 28 layers, 4 projs each, 2 matrices each = 224 params tensors
...
train_data = [
    "This is the first sample that i will use to train lora adapter. Bless me!",
    "This is the second sample.",
]

tokenizer.pad_token = tokenizer.eos_token
inp = tokenizer(
    train_data, return_tensors="pt", padding=True, truncation=True, max_length=64
)
