...
class GPTJAttentionWithHooks(GPTJAttention):
    ...
    def _register_forward_hooks(self):
        ...
        def module_forward_hook(name):
            def hook(module, input, output):
                ...
                if name == "Q":
                    self.weight_snapshots["Q"]["before_backprop"] = (
                        self.q_proj.weight.data[0, :5].clone()
                    )
                elif name == "K":
                    self.weight_snapshots["K"]["before_backprop"] = (
                        self.k_proj.weight.data[0, :5].clone()
                    )
                elif name == "V":
                    self.weight_snapshots["V"]["before_backprop"] = (
                        self.v_proj.weight.data[0, :5].clone()
                    )
    ...
    def capture_weights_after_optimizer(self):
        ...
        self.weight_snapshots["Q"]["after_optimizer"] = self.q_proj.weight.data[
            0, :5
        ].clone()
        self.weight_snapshots["K"]["after_optimizer"] = self.k_proj.weight.data[
            0, :5
        ].clone()
        self.weight_snapshots["V"]["after_optimizer"] = self.v_proj.weight.data[
            0, :5
        ].clone()
    ...
    def print_summary(self):
        ...
        for name in ["Q", "K", "V"]:
            if (
                self.weight_snapshots[name]["after_optimizer"] is not None
                and self.weight_snapshots[name]["before_backprop"] is not None
            ):
                change = (
                    self.weight_snapshots[name]["after_optimizer"]
...
class GPTJModelWithHooks(GPTJModel):
    ...
    def __init__(self, config, track_layer_idx=0):
        ...
        self.h = nn.ModuleList(
            [
                GPTJBlockWithHooks(
                    config, layer_idx=i, enable_hooks=(i == track_layer_idx)
                )
...
class GPTJForCausalLMWithHooks(GPTJForCausalLM):
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path, track_layer_idx=0, *args, **kwargs
    ):
        ...
        )  # Note: copy references only for saving memory
        ...
        new_block = GPTJBlockWithHooks(
            config, layer_idx=track_layer_idx, enable_hooks=True
        )  # Note: we are only creating new memory for 1 layer
